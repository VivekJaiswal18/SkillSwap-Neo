...
export const CONTRACT_ADDRESS = "0xf6d3a05d646432107d37dfe3320f4e1d557736de"; //aiachain.sol
...
// 					}
// 				],
// 				"internalType": "struct EducationalMarketplace.Course[]",
// 				"name": "",
// 				"type": "tuple[]"
// 			}
// 		],
...
// 	const tx = await contract.publishCourse(price, contentURI);
// 	await tx.wait();
// 	return tx;
//   };
...
export const publishCourse = async (signer: ethers.Signer, price: bigint, contentURI: string) => {
	const contract = getContract(signer);
	const tx = await contract.publishCourse(price, contentURI);
	await tx.wait();
	return tx;
...
// export const getAllCourses = async (provider: ethers.Provider) => {
...
export const getNextCourseId = async (provider: ethers.Provider) => {
	const contract = getContract(provider);
	return await contract.nextCourseId();
  };
  
//   export const getCourse = async (provider: ethers.Provider, courseId: number) => {
// 	const contract = getContract(provider);
// 	return await contract.courses(courseId);
//   };
...
export const getAllCourses = async () => {
  ...
  export const purchaseCourse = async (signer: ethers.Signer, courseId: number, price: bigint) => {
	const contract = getContract(signer);
